...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "data" / "mt5_segmented_v2"
CHUNK_SPLIT_RE = re.compile(r"^Chunk\s+(\d+):\s*$", re.MULTILINE)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare article-split mT5 samples from segmented_v2 chunk summaries."
    )
...
def main():
    ...
    stats = {
        ...
        "seed": args.seed,
        "segment_sum_concat_files": len(concat_files),
        "raw_pairs": raw_pairs,
