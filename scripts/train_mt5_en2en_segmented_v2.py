...
DEFAULT_TRAIN_FILE = PROJECT_ROOT / "data" / "mt5_en2en_segmented_v2" / "train.jsonl"
DEFAULT_TEST_FILE = PROJECT_ROOT / "data" / "mt5_en2en_segmented_v2" / "test.jsonl"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "mt5-en2en-segmented-v2-epoch1"


def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune mT5 on EN->EN segmented_v2 samples.")
    ...
    parser.add_argument(
        "--generate_only",
        action="store_true",
        help="Skip training and load the model from --output_dir to generate on the test set.",
    )
    parser.add_argument(
        "--fp16",
...
def main():
    ...
    load_path = args.output_dir if args.generate_only else args.model_name
